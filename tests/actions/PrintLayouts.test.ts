...
describe("PrintLayouts.execute - Umfassende Tests", () => {
  ...
  describe("GET_ALL Operation", () => {
    ...
    it("sollte komplexe Layout-Strukturen handhaben", async () => {
      ...
      expect(result[0].json.settings.header.logo).toBe(true);
      expect(result[0].json.settings.colors.primary).toBe("#003366");
    ...
    it("sollte Layout-Metadaten korrekt verarbeiten", async () => {
      ...
      // Assert
      expect(result[0].json.metadata.version).toBe("2.1.0");
      expect(result[0].json.metadata.tags).toContain("modern");
      expect(result[0].json.metadata.usageCount).toBe(147);
    });
  ...
  describe("Performance Tests", () => {
    ...
    it("sollte bei wiederholten Aufrufen konsistent performen", async () => {
      ...
      expect(maxTime - minTime).toBeLessThan(avgTime * 2); // Variation unter 200%
